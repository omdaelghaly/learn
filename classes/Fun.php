...
public function checkIfExistCat($column,$table,$value,$parent_id) {
         $search =  "SELECT $column FROM $table WHERE $column = '$value' && parent_id='$parent_id' ";
         $rows=  mysqli_query($this->connect(),$search);
         $numRows = mysqli_num_rows($rows);
...
public function validateSize($files,$maxSize)
{
    foreach ($files['name'] as $index => $file_name) {
        ...
        if ($filesize > $maxSize) {
            return $filesize; // error in this filename
